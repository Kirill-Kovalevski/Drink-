// ===== דרינק! — מחשבון מים + גל מונפש + מד נהר + תזכורות =====
...
const state=JSON.parse(localStorage.getItem('drink.v10')||'{}');
...
const wMinus=$('#wMinus'), wPlus=$('#wPlus'), aMinus=$('#aMinus'), aPlus=$('#aPlus');

/* Ripple hero animation */
(function ripples(){
  const cvs=$('#rippleHero'); if(!cvs) return; const ctx=cvs.getContext('2d');
  let w,h,t=0; const waves=[{amp:14,spd:.015,color:'#7dd3fc'},{amp:10,spd:.022,color:'#38bdf8'},{amp:8,spd:.030,color:'#60a5fa'}];
  ...
  (function draw(){t+=1; ctx.clearRect(0,0,w,h); ctx.fillStyle='#eaf6ff'; ctx.fillRect(0,0,w,h);
    waves.forEach((wv,i)=>{ctx.beginPath(); for(let x=0;x<=w;x+=2){const y=h*0.58+Math.sin(x*0.02+t*wv.spd)*wv.amp+Math.cos(x*0.04+t*wv.spd*1.3)*2; if(x===0)ctx.moveTo(x,y); else ctx.lineTo(x,y);} ctx.lineTo(w,h); ctx.lineTo(0,h); ctx.closePath(); ctx.fillStyle=wv.color; ctx.globalAlpha=.85-i*.22; ctx.fill();});
    ctx.globalAlpha=1; requestAnimationFrame(draw);})();
})();

/* Quotes (קצרים מהתנ"ך) — מתחלפים לפי התקדמות */
const QUOTES=[
...
/* Target calculation (מבוסס AI/NIH + התאמות): 
   זכר 3.7ל', נקבה 2.7ל', פעילות +0.2ל' לכל 30דק, אקלים חם +0.5ל', 
   הריון +0.3ל', הנקה +0.7ל', קפה ~100מ״ל לכל כוס (מוסיף 0.1ל'),
   אלטיטודה בינונית/גבוהה +0.3/+0.7ל', התאמה קלה לפי BMI. */
function calcTargetMl(){
  const s=sex.value, kg=kgFrom(+weight.value||0, unit.value);
...
function render(){
  ...
  localStorage.setItem('drink.v10', JSON.stringify(state));
}
...
/* Steppers */
wPlus.addEventListener('click', ()=>{ const step=(unit.value==='kg'?0.5:1); weight.value=(+weight.value||0)+step; onAnyChange(); });
wMinus.addEventListener('click', ()=>{ const step=(unit.value==='kg'?0.5:1); weight.value=Math.max(0,(+weight.value||0)-step); onAnyChange(); });
